...
5. **환자 피부 = SKEL 토폴로지**(`ct2skel/patient.py`) — 정합된 SKEL 피부(6,890 정점)를 중점 분할 2회(약 11만 정점)한 뒤 정점을 CT 체표로 이동시킵니다(메시 그래프에서 변위장 평활화, KD-tree 기반). 스키닝 가중치는 SKEL 것을 분할 시 보간해 그대로 쓰므로 자세 변경이 SKEL과 똑같이 동작하고, marching cubes 메시에 최근접 정점으로 가중치를 옮기던 방식의 찢어짐·늘어남이 원천적으로 없습니다. CT가 없는 구간과 추정 사지는 파라미터 피부 그대로입니다. 원본 marching cubes 표면은 `ct_surface_raw`(숨김, 자세 변경 안 됨)로 남깁니다. `--skin-subdiv`, `--no-skin-refine`
   ...
   **CT 원생 피부(기본, 라벨이 있을 때)** (`ct2skel/volweights.py`): 환자 자신의 CT 체표 메시를, 환자 자신의 뼈(라벨 뼈 + 분류된 조각)를 열원으로 체표 마스크 안에서 확산시킨 **부피 스키닝 가중치**로 움직입니다(뼈에서 Dirichlet, 피부에서 Neumann; 3 mm 격자, GPU 확산 400회, 수 초). 통계 모델의 피부 토폴로지·가중치가 CT가 있는 곳에서는 전혀 쓰이지 않으므로 손가락·발·비만 몸통 모두 CT 형상 그대로이고, 변형은 이 환자의 뼈·연부조직 분포를 따릅니다. CT 밖 구간과 추정 사지에만 SKEL 토폴로지 피부를 이어 붙입니다. `--no-volumetric-skin` 으로 이전 방식(SKEL 토폴로지 + CT 손·발) 사용.
