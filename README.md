...
5. **환자 피부 = SKEL 토폴로지**(`ct2skel/patient.py`) — 정합된 SKEL 피부(6,890 정점)를 중점 분할 2회(약 11만 정점)한 뒤 정점을 CT 체표로 이동시킵니다(메시 그래프에서 변위장 평활화, KD-tree 기반). 스키닝 가중치는 SKEL 것을 분할 시 보간해 그대로 쓰므로 자세 변경이 SKEL과 똑같이 동작하고, marching cubes 메시에 최근접 정점으로 가중치를 옮기던 방식의 찢어짐·늘어남이 원천적으로 없습니다. CT가 없는 구간과 추정 사지는 파라미터 피부 그대로입니다. 원본 marching cubes 표면은 `ct_surface_raw`(숨김, 자세 변경 안 됨)로 남깁니다. `--skin-subdiv`, `--no-skin-refine`
   **관절 목표의 원칙**: 관절 위치 목표는 사지 축 위에 있는 관절(팔꿈치·손목·무릎·발목)에만 줍니다. 요골·종골·발가락 관절은 그룹의 (관측 불가한) 비틀림으로만 움직이므로 목표에서 빼고, 손·발은 위치만 약하게 맞추며 자세는 사전에서 옵니다. 비틀림 DOF(고관절 회전, 전완 회내외)는 촬영 여부와 무관하게 누운 자세 사전을 유지합니다. SKEL에 없는 고관절 가동범위(굴곡 −30~120°, 내전 ±45°, 회전 ±60°)를 추가했습니다. 앉기·서기·고관절 프리셋은 고관절 회전·내전을 중립으로 되돌립니다(누운 시신은 발이 바깥으로 벌어져 있음).
   **긴 뼈의 비틀림**: 원통에 가까운 뼈의 ICP는 축 회전(비틀림)에 국소 최소가 여럿입니다. 대퇴·경골·상완·척골·요골(및 라벨 없는 전완·종아리·손·발 그룹)은 축 둘레 6개 시작 각(0, ±60, ±120, 180°)에서 ICP를 돌려 최선을 고르고(`refine.icp_multistart`), 최선과 차선의 잔차 차이(`twist_margin`)가 15 % 미만이면 데이터가 비틀림을 구별하지 못하는 것이므로 그 뼈의 방향 목표는 축 방향만 구속합니다. 이전엔 잘못된 비틀림 목표를 따라가 고관절 회전이 −73°/97°가 되는 문제가 있었습니다.
   **사지 말단의 운동학**: 라벨이 없는 전완·종아리 조각에 척골+요골 / 경골을 ICP로 맞춘 뒤, 같은 조각에 손(`hand`)과 발(`talus+calcn+toes`) 그룹도 이어서 맞춰 손목·발목 자세를 CT에서 얻습니다. 촬영 범위 판정(DOF freeze)은 모델이 놓이기 전엔 8 cm 여유로, 이후 단계마다 다시 평가합니다(예전엔 초기 배치에서 발이 FOV 밖으로 나가 무릎·발목이 통째로 고정되는 문제가 있었음).
   **피부 대응점**: 환자 피부 정합은 법선이 일치하고(내적 > 0.5) 거리가 단계적으로 줄어드는 한계(80→40 mm) 안에 있는 CT 점만 대응으로 씁니다(Amberg 2007식 비강체 ICP). 허벅지에 얹힌 손의 피부가 허벅지 표면으로 빨려 들어가는 일을 막습니다.
   **CT 원생 피부(기본, 라벨이 있을 때)** (`ct2skel/volweights.py`): 환자 자신의 CT 체표 메시를, 환자 자신의 뼈(라벨 뼈 + 분류된 조각)를 열원으로 체표 마스크 안에서 확산시킨 **부피 스키닝 가중치**로 움직입니다(뼈에서 Dirichlet, 피부에서 Neumann; 3 mm 격자, GPU 확산 400회, 수 초). 통계 모델의 피부 토폴로지·가중치가 CT가 있는 곳에서는 전혀 쓰이지 않으므로 손가락·발·비만 몸통 모두 CT 형상 그대로이고, 변형은 이 환자의 뼈·연부조직 분포를 따릅니다. 확산만으로는 CT에서 팔이 몸통에 닿아 있는 곳에서 가중치가 새므로(팔을 올리면 가슴 피부가 딸려 올라감), 각 피부 정점의 부위(CT에 맞춘 SKEL 피부가 12 mm 이내면 그 부위, 아니면 최근접 CT 뼈의 부위)와 그 운동학적 이웃(부모·자식)에만 가중치를 허용하는 해부학적 게이트를 겁니다(`volweights.gate_weights_by_part`). CT 밖 구간과 추정 사지에만 SKEL 토폴로지 피부를 이어 붙입니다. `--no-volumetric-skin` 으로 이전 방식(SKEL 토폴로지 + CT 손·발) 사용.
   **손·발 피부는 CT 그대로**(SKEL 토폴로지 모드에서): SKEL의 손·발은 덩어리 모형이라 벌린 손가락·굽은 발을 따라갈 수 없으므로, CT 피부에서 손·발 부분(가장 가까운 CT 뼈가 손·발 뼈인 정점)을 잘라내 손·발 뼈에 강체로 붙이고 SKEL 토폴로지 피부는 그 부위 면을 뺍니다(`patient.merge_extremity_skin`). 추정 사지는 SKEL 그대로입니다. 결과 환자 피부는 몸통·팔·다리(SKEL 스키닝) + 손·발(CT 형상, 강체)의 하이브리드입니다.
   **해부학적 게이트**: 첫 정합 뒤 정합된 체형 모델에서 `--gate-mm`(기본 35 mm) 이상 떨어진 CT 피부·뼈·라벨 없는 조각은 환자가 아니므로 버립니다(테이블, QA 팬텀, 케이블, 와이어). 라벨 뼈는 연결 성분 단위로 *자기 SKEL 뼈*와의 거리로 검사해 분할 잡음(섬)을 지웁니다. 어떤 촬영 장비·프로토콜에서도 같은 규칙이 적용됩니다.
   **검증**: `powershell -File scripts/viewer_shots.ps1 -Url http://127.0.0.1:8000/index.html -Poses ct,sitting,arms_up` 로 헤드리스 크롬 스크린샷을 찍어 CT 자세와 프리셋 자세를 눈으로 확인합니다.
6. **환자 뼈**: 라벨 마스크에 닫힘(2.5 mm)·구멍 채우기·섬 제거·Taubin 30회를 적용한 매끈한 CT 뼈(`ct_bone_<part>`); 늑연골은 `ct_cartilage`로 분리(기본 숨김). 라벨이 없는 뼈 밀도 덩어리는 8 mm 이내면 이웃 라벨에 붙이고, 큰 조각은 `ct_bone_unlab_*`로 따로 둡니다. 단, 한 라벨 뼈의 상하 범위 안에 있고 그 뼈에 닿는 큰 조각(라벨이 골간을 빠뜨린 경우)은 그 뼈로 되돌리며, 조각 메시의 작은 성분(면적 400 mm²·크기 15 mm 미만: 석회화 점 등)은 버립니다. Taubin 평활화는 균일 라플라시안 구현(`meshing.taubin_smooth`)을 씁니다 — trimesh 내장 `filter_taubin` 은 marching cubes 가 남기는 비다양체 모서리에서 정점을 수십 cm 튕겨내 가시 모양 슬리버를 만듭니다. 면 수 축소 뒤에는 안전장치로 극단적인 슬리버 삼각형도 제거합니다(`meshing.remove_sliver_faces`). CT 범위 밖은 이 환자 체형에 맞춘 SKEL 뼈가 "추정" 그룹으로 이어짐. `ct2skel refresh --out DIR` 로 가중치(뼈·CT 피부)·뷰어만 재생성
7. **자세 변경**(`ct2skel/pose.py`) — 뼈도 SKEL 골격 스키닝 가중치(`skel_weights`, 같은 부위의 SKEL 골격 정점 중 최근접; 견갑골 아래 늑골이나 견봉이 이웃 뼈에 묶이지 않도록 부위를 제한. 라벨 없는 조각(종아리+발, 전완+손)은 그 사지 사슬 안에서 정점마다 분류해 발뼈는 발목을, 손뼈는 손목을 따라감)로 움직이므로 사지는 강체, 요추·흉곽은 SKEL처럼 연속적으로 휘고 관절면이 벌어지지 않음 (`bone_weights.bin`): SKEL 운동학으로 분절별 강체 변환 M_j = G'_j·G_j⁻¹ 를 계산해 CT 뼈·템플릿 뼈는 강체로, 피부는 SKEL 스키닝 가중치(CT 피부는 최근접 SKEL 정점 가중치)로 움직임. `poses.json` 에 프리셋(팔 내림/올림, 무릎 90°, 고관절 45°, 앉기, 몸통 굴곡, 머리 회전)과 11단계 보간 프레임 저장. 뷰어 "Pose" 섹션에서 선택·보간
   ```powershell
